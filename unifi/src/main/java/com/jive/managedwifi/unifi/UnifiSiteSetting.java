...
public class UnifiSiteSetting extends UnifiBase implements SiteSetting {
	...
	public String updateTou(final String sessionId, final Map form)
		 ...
		 * Note to maintainers:
		 * 
		 * This requires a couple of changes to visudo on the server: 1. comment
		...
		final String[] cmd = { "/bin/bash", "-c",
				"sudo chmod -R 777 /usr/lib/unifi" };
		final Process pb = Runtime.getRuntime().exec(cmd);
		pb.waitFor();
		String line;
		String output = "";
		...
		String outErr = "";
		final BufferedReader inputErr = new BufferedReader(
		...
		String html = IOUtils.toString(Thread.currentThread()
				.getContextClassLoader().getResourceAsStream("template.txt"));
		...
		return "deleted: " + deleted + " wrote: " + wrote + " log: " + output
				+ outErr;
	}
	...
	public Map updateLimits(final String sessionId, final String userGroupId,
		...
		return postFormData(
				...
				String.format("/api/s/%s/upd/usergroup/%s",
						siteInfo.get("name"), userGroupId), limits);
	}
}
