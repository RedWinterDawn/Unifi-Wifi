...
public class UnifiSite extends UnifiBase implements Site {
    ...
    @Override
    public List<Object> getSites(final String sessionId) {
        ...
        final BasicDBObject query = new BasicDBObject("account_id", session.get("account_id"));
    ...
    @Override
    public void update(final String id, final Map site) {
        site.remove("_id");
        final DB db = dbClient.getDB("ace");
        final DBCollection dbCollection = db.getCollection("site_ext");
        final BasicDBObject query = new BasicDBObject("site_id", id);
  ...
  @SuppressWarnings("rawtypes")
    public String add(final String sessionId, final Map site){
        ...
        final WebTarget target = client.target(controllerHost + "/api/s/super/cmd/sitemgr");
        final Response response = target
                .request(MediaType.APPLICATION_FORM_URLENCODED)
                .cookie("unifises", sessionId)
                .post(Entity.form(form));
        final Map newSite = response.readEntity(Map.class);
        final String newSiteId = ((List<Map>) newSite.get("data")).get(0).get("_id").toString();
    ...
    public void makeActive(final String sessionId, final Map site){
        final Map<String, Object> session = getSessionInfo(sessionId); //authenticates
    ...
    private void makeActive(final String sessionId, final String siteId){
        ...
        final BasicDBObject update = new BasicDBObject("$set", new BasicDBObject("site_id", siteId));
